...
sudo service ntp stop # Restart the ntp service so that only the ntp.statsbiblioteket.dk is used
sudo ntpdate ntp.statsbiblioteket.dk # And make sure the date is now set from ntp server
